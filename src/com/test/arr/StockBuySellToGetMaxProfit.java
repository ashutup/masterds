...
public class StockBuySellToGetMaxProfit {
	...
	public static void analyze(int[] price, int noOfDays) {
		int i = 0;

		while (i < noOfDays - 1) {

			while (i < noOfDays - 1 && price[i + 1] <= price[i]) {
				i++;
			}
			...
			while (i < noOfDays && price[i - 1] < price[i])
				i++;

			int sell = i - 1;

			System.out.println("buy at " + price[buy] + " and sell at "
					+ price[sell]);
